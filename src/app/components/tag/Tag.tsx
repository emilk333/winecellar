...
export const Tag: FC<TagProps> = (tagProps) => {
    ...
    return(
        <div className={`h-full absolute left-[-40px] top-[15px] after:content-[''] after:h-[calc(100%_-_40px)] after:w-[2px] after:absolute after:left-[5px] after:top-[10px] ${pseudoColor}`}>
            ...
            <div className={`z-20 top-[-5.5px] left-[-2px] capitalize h-fit w-[16px] py-0.5 ${color} no-height-change-rotate relative z-10`}>
                <span className={`relative left-[4px] text-[10px] font-bold ${accentColor}`}>{data}</span>
            </div>
